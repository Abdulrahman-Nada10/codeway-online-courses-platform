...
export default function AssignmentsPanel({ assignments }: { assignments: LessonAssignment[] }) {
  ...
  if (assignments.length === 0) {
    return (
      <div className="flex h-[176px] flex-col items-center justify-center rounded-[16px] border border-[#E8D8CA] bg-white text-[#8B8B8B] shadow-[0_10px_24px_rgba(17,53,85,0.06)]">
  ...
  return (
    <div className="h-[176px] overflow-hidden rounded-[16px] border border-[#E8D8CA] bg-white px-5 py-3 shadow-[0_10px_24px_rgba(17,53,85,0.06)]">
      <div className="custom-scrollbar h-full space-y-4 overflow-y-auto pr-1">
        {assignments.map((assignment) => {
          ...
          return (
            <div key={assignment.id} className="flex items-start justify-between gap-3 rounded-[12px] py-1">
              <button
                type="button"
                onClick={() =>
              ...
              >
                {isDownloaded ? (
                  <span className="inline-flex items-center">
                    ...
                    <span className="mt-5.5 inline-block w-5 border-b border-current absolute" />
                  </span>
                ) : (
