...
const AchievementCard = ({
  ...
  <article className="flex items-center gap-4 bg-[#FDFBF7] p-5 rounded-2xl border border-gray-100/50">
    <div className="shrink-0 flex items-center justify-center w-12 h-12 rounded-full bg-orange-50 text-orange-500">
      <Icon size={24} strokeWidth={2} aria-hidden="true" />
    </div>
...
export default function ProfileContent() {
  ...
  const instructorPhone = user?.phoneNumber ?? '';
  const instructorField = user?.role === 'instructor' ? user.field : '';
  const instructorJoinTime = user?.joinTime ?? '';
  const instructorAvatar = user?.avatar ?? '/profile.jpg';
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h2 className="text-xl font-bold text-gray-900">{instructorName}</h2>
                <span className="bg-green-500 text-white text-xs font-semibold px-3 py-1 rounded-full">نشط</span>
              </div>
              <p className="text-sm text-gray-500">{instructorField}</p>
            </div>
          </div>
          ...
          <div className="w-full md:w-auto grid grid-cols-1 sm:grid-cols-2 lg:flex lg:flex-row gap-4 lg:gap-8 text-sm text-gray-600">
            <div className="flex items-center gap-2">
              ...
              <span dir="ltr">{instructorEmail}</span>
            </div>
            <div className="flex items-center gap-2">
              <Phone size={16} className="text-orange-500" />
              <span dir="ltr">{instructorPhone}</span>
            </div>
            <div className="flex items-center gap-2">
              <FileBracesCorner size={16} className="text-orange-500" />
              <span>{instructorField}</span>
            </div>
            <div className="flex items-center gap-2">
              <CalendarDays size={16} className="text-orange-500" />
              <span>انضم في {instructorJoinTime}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <section className="lg:col-span-7 bg-white rounded-2xl p-6 shadow-sm border border-gray-100" aria-labelledby="bio-title">
          ...
          <p className="text-sm text-gray-600 leading-loose">
            مدرب وخبير متخصص في مجال البرمجة وتطوير البرمجيات. يمتلك خبرة عملية وتدريبية تمتد لأكثر من 10 سنوات في تأهيل الكوادر التقنية وبناء المطورين المحترفين.
            <br className="my-2" />
            يركز أسلوبه التدريبي على الشرح العملي، المشاريع التطبيقية، وربط المحتوى بمتطلبات سوق العمل.
          </p>
        </section>
