...
export default function Certificates() {
  ...
  const filteredCertificates = useMemo(() => {
    ...
    return certificates.filter(
      (cert) =>
        cert.title.toLowerCase().includes(lower) ||
        cert.instructor.toLowerCase().includes(lower) ||
        cert.courseName.toLowerCase().includes(lower)
    );
  }, [searchQuery]);
    ...
    <div className="min-h-screen bg-[#FFF3EB] overflow-x-hidden mt-26">
      <div className="lg:mr-70 xl:mr-70 md:mr-0">
        <main className="p-2 sm:p-3 lg:p-6">
          ...
          <div className="w-full max-w-273 h-85.25 border border-solid mt-8 sm:mt-12 bg-white rounded-3xl p-2.5 sm:p-6 shadow-sm flex flex-col items-center justify-center">
            <div className="w-112.25 height-76.5 flex flex-col items-center justify-center gap-6">
              ...
              <div className="text-center">
                ...
                <p className="font-cairo text-sm sm:text-base text-gray-600 mt-1">
                  لديك <span className="font-bold text-[#FF6400]">{filteredCertificates.length}</span> شهادات حتى الآن
                </p>
