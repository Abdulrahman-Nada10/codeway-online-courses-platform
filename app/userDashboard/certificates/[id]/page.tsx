...
import { getUserCertificateById } from '../data';
...
export default function CertificateDetail() {
  ...
  const certificateData = getUserCertificateById(id, t);

  return (
    <div
      className="mt-26 min-h-screen bg-[#FFF3EB] dark:bg-slate-950"
      dir={dir}
    >
        ...
        <div className="p-3 sm:p-6">
          <div className="mb-6 flex items-center gap-3">
            <Link
              ...
              className="flex h-8 w-8 items-center justify-center rounded-lg bg-white shadow-sm transition-shadow hover:shadow-md dark:bg-slate-900"
            >
              <svg
                width="16"
                height="16"
            ...
            <span className="font-cairo text-sm text-[#113555] dark:text-slate-100">
              {t('certificates.backToCertificates')}
            </span>
          </div>

          <div className="overflow-hidden rounded-2xl bg-[linear-gradient(109.45deg,#FF6400_0.67%,#F59F00_99.65%)] p-1.25">
            <div className="w-full rounded-xl bg-white px-6 py-10 dark:bg-slate-900">
              <div className="flex flex-col items-center gap-8">
                <div className="flex h-30 w-30 items-center justify-center rounded-full bg-linear-to-b from-[#FF6400] to-[#F5A00F] p-3">
                  <svg
                  ...
                  >
                    <path
                      d="M24 4L8 12V24C8 33.7 14.8 42.76 24 45C33.2 42.76 40 33.7 40 24V12L24 4Z"
                      stroke="currentColor"
                      strokeWidth="3"
                      strokeLinejoin="round"
                    />
                    <path
                      d="M18 23L22 27L31 18"
                      stroke="currentColor"
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                ...
                <div className="text-center">
                  <p className="mb-2 font-cairo text-sm text-gray-500 dark:text-slate-400">
                  ...
                  <div className="mx-auto mt-3 h-1 w-45 rounded-full bg-linear-to-r from-[#FF6400] to-[#F59F00]" />
                </div>

                <div className="text-center">
                  <p className="mb-1 font-cairo text-sm text-gray-500 dark:text-slate-400">
                    {t('certificates.awardedTo')}
                  </p>
                  <h2 className="font-cairo text-lg font-bold text-[#113555] dark:text-slate-100 sm:text-xl">
                    {certificateData.studentName}
                  </h2>
                  <p className="mx-auto mt-2 max-w-md font-cairo text-sm text-gray-600 dark:text-slate-400">
                    {t('certificates.completionDescription', {
                      course: certificateData.courseName,
                      hours: certificateData.totalHours,
                      units: certificateData.units,
                    })}
                  </p>
                </div>

                <div className="flex w-full max-w-3xl flex-wrap justify-center gap-4">
                  <div className="flex min-h-22.5 min-w-35 flex-1 flex-col items-center justify-center gap-1 rounded-lg bg-[#FFF3EB] p-3 shadow-[0px_2px_2px_0px_rgba(0,0,0,0.25)] dark:bg-slate-800">
                    ...
                    <p className="font-cairo text-xs text-gray-500 dark:text-slate-400">
                      {t('dashboard.issueDate')}
                    </p>
                    <p className="font-cairo text-xs font-bold text-[#113555] dark:text-slate-100">
                  ...
                  <div className="flex min-h-22.5 min-w-35 flex-1 flex-col items-center justify-center gap-1 rounded-lg bg-[#FFF3EB] p-3 shadow-[0px_2px_2px_0px_rgba(0,0,0,0.25)] dark:bg-slate-800">
                    ...
                    <p className="font-cairo text-xs font-bold text-black dark:text-slate-100">
                      {certificateData.status}
                    </p>
                  </div>
                </div>

                <div className="mt-2 flex flex-wrap justify-center gap-8 sm:gap-16">
                  <div className="flex flex-col items-center gap-2">
                    <div className="h-0.5 w-45 rounded-[20px] bg-[rgba(255,198,161,1)]" />
                    <p className="font-cairo text-md font-bold text-black dark:text-slate-100">
                      {t('dashboard.instructorPrefix')}{certificateData.instructor}
                    </p>
                    <p className="font-cairo font-semibold text-gray-600 dark:text-slate-400">
                      {t('certificates.instructor')}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <div className="h-0.5 w-45 rounded-[20px] bg-[rgba(255,198,161,1)]" />
                    <p className="font-cairo text-md font-bold text-black dark:text-slate-100">
                      {t('certificates.platformManagement')}
                    </p>
                    <p className="font-cairo text-md font-semibold text-gray-600 dark:text-slate-400">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <button className="flex items-center gap-2 rounded-lg bg-[#FF6400] px-6 py-3 font-cairo text-sm font-semibold text-white transition-colors hover:bg-[#E55A00]">
