...
export function QuestionCard({ currentUserId, question }: QuestionCardProps) {
  ...
  return (
    <article className="rounded-2xl border border-[#ffd8bf] bg-[#fff8f4] p-4 text-start dark:bg-slate-800" dir={dir}>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-[#ff6400] px-2.5 py-1 text-[11px] text-white">{t('dashboard.questionForInstructor')}</span>
              {isCurrentUser ? <span className="rounded-full bg-white px-2 py-1 text-[11px] text-[#ff6400]">{t('nav.you')}</span> : null}
            </div>
          ...
          <div className="mt-3 rounded-2xl bg-white px-4 py-3 text-sm leading-7 text-[#6b7280] shadow-[0_8px_30px_rgba(17,53,85,0.05)]">
            <div className="mb-2 flex items-center gap-2 text-xs text-[#ff6400]">
              <span>{t('dashboard.directedToInstructor')}</span>
