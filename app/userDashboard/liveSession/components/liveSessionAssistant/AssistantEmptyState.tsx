...
import { introSuggestions } from './constants';
...
export function AssistantEmptyState({
  ...
  return (
    <div className="flex h-full min-h-[320px] flex-col items-center justify-center text-center sm:min-h-[380px]">
      ...
      >
        <h2 className="text-3xl font-bold text-[#1f2937] sm:text-4xl">إزاي أقدر أساعدك؟</h2>
      </div>
      ...
      >
        {introSuggestions.map((suggestion) => (
          <button
            ...
            type="button"
            disabled={isLoading}
            onClick={() => onSuggestionClick(suggestion.buildPrompt(contextTitle), suggestion.label)}
