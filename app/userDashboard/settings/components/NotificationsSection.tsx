...
function NotificationToggle({
  ...
  return (
    <div className="flex flex-col justify-between gap-4 rounded-xl bg-background p-4 dark:bg-background sm:flex-row sm:items-center">
      <div className="text-start">
        <h4 className="font-cairo text-base font-medium text-main-text dark:text-main-text">
          {title}
        </h4>
        <p className="mt-1 font-cairo text-sm text-muted-foreground dark:text-muted-foreground">
          {description}
        </p>
      </div>
      <button
        ...
        className={`relative w-18 h-8 rounded-full transition-all duration-300 shrink-0 ${
          enabled ? "bg-primary" : "bg-input-bg"
        }`}
        aria-label={`${title}: ${
      ...
      >
        <span
          className={`absolute top-1 w-6 h-6 bg-background rounded-full shadow-md transition-all duration-600 ${
            enabled
              ? isRTL
                ? "left-1"
                : "right-1"
