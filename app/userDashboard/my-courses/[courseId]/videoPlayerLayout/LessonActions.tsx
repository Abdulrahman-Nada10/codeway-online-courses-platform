...
export default function LessonActions({
  ...
  return (
    <div className="flex flex-col gap-2 rounded-2xl border border-[#E8D8CA] bg-white px-3 py-3 shadow-[0_8px_20px_rgba(17,53,85,0.05)] dark:bg-slate-900 sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-4" dir={dir}>
      <h2 className="text-start text-[12px] font-bold text-[#111111] dark:text-slate-100 min-[360px]:text-[13px] sm:text-[14px]">{title}</h2>
      ...
      <div className="flex flex-wrap items-center gap-3 text-[10px] min-[360px]:gap-4 min-[360px]:text-[11px] sm:justify-start">
        ...
        >
          ...
          {status === 'completed' ? t('dashboard.markIncomplete') : t('dashboard.markComplete')}
        </button>
      </div>
    </div>
  );
